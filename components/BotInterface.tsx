...
interface BotInterfaceProps {
  ...
  onOpenStrategicAnalysis: () => void;
}

const BotInterface: React.FC<BotInterfaceProps> = ({ status, onStatusChange, onBroadcast, isBroadcasting, onOpenStrategicAnalysis }) => {
  ...
  return (
    <div className={`border bg-slate-950/80 backdrop-blur-2xl p-5 rounded-xl transition-all duration-700 relative overflow-hidden ${colorMap[theme]} shadow-2xl blueprint-border`}>
      ...
      <form onSubmit={handleBroadcastSubmit} className="mb-4 relative z-10">
        <div className="text-[7px] text-slate-500 font-mono uppercase mb-1 flex justify-between">
          <span>System Broadcast Uplink</span>
          {isBroadcasting && <span className="text-cyan-400 animate-pulse">TRANSMITTING...</span>}
        </div>
        <div className="flex gap-2">
          <input
            ...
            placeholder="Enter message for RAI Core..."
            className="flex-1 bg-black/40 border border-slate-800 rounded px-2 py-1.5 text-[10px] text-slate-300 placeholder:text-slate-700 focus:outline-none focus:border-cyan-500/50 transition-all font-mono"
          ...
          <button
            ...
            className={`px-3 rounded border transition-all flex items-center justify-center ${
              isBroadcasting 
                ...
                : 'bg-slate-800 border-slate-700 text-slate-500 hover:text-cyan-400 hover:border-cyan-500/50'
            }`}
          >
      ...
      {/* Strategic Analysis Trigger */}
      <button 
        onClick={onOpenStrategicAnalysis}
        className="w-full mb-4 py-2 bg-cyan-500/10 border border-cyan-500/30 rounded-lg text-[9px] font-orbitron font-bold text-cyan-400 hover:bg-cyan-500/20 transition-all flex items-center justify-center gap-2 group"
      >
        <span className="group-hover:animate-pulse">🛡️</span>
        INITIATE_STRATEGIC_UPLINK
      </button>
