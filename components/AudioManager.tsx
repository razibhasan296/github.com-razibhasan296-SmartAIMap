...
interface AudioManagerProps {
  ...
  isMuted: boolean;
  ttsAudio?: string; // Base64 audio data
}

const AudioManager: React.FC<AudioManagerProps> = ({ profile, status, scrollPos, isMuted, ttsAudio }) => {
  ...
  // Handle TTS Playback
  useEffect(() => {
    if (!ttsAudio || !audioCtxRef.current || !ttsGainRef.current) return;
  ...
  useEffect(() => {
    ...
    switch (profile) {
      case 'Natural': // Wind soundscape
        ...
        // Harmonic drones: C3 (130.81Hz), G3 (196.00Hz), E2 (82.41Hz)
        bank[0].type = 'sine'; bank[0].frequency.setTargetAtTime(130.81, now, 0.5);
        bank[1].type = 'sine'; bank[1].frequency.setTargetAtTime(196.00, now, 0.5);
        bank[2].type = 'sine'; bank[2].frequency.setTargetAtTime(82.41, now, 0.5);

        // Soft fade in for drones in Natural profile
        dGain.gain.cancelScheduledValues(now);
        dGain.gain.setValueAtTime(0, now);
        dGain.gain.setTargetAtTime(0.25, now, 2.0);
