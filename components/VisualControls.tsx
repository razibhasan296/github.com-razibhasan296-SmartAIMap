              ...
              {modes.map(mode => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="text-[10px]">{mode === 'Street View' ? '🚶' : mode === 'Satellite View' ? '🛰️' : mode === '3D City Model' ? '🏙️' : '🏢'}</span>
                    <span>{mode.toUpperCase()}</span>
                  </div>
                  {selectedMode === mode && <span className="w-1 h-1 theme-bg-primary rounded-full animate-pulse shadow-[0_0_5px_var(--theme-primary)]"></span>}
                </button>
              ))}
            </div>
          </div>
