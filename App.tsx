...
import { generateLocationData, generate360View, generateMultiSpeakerAudio } from './services/geminiService';
...
const VISUAL_STYLES: VisualStyle[] = ['Cinematic', 'Thermal', 'Night Vision', 'Cyberpunk', 'Deep Space', 'Post-Apocalyptic', 'Solar-Punk', 'Diplomatic', 'Codex'];
...
const THEME_CONFIGS: Record<VisualStyle, { primary: string, secondary: string, accent: string, bg: string, glow: string }> = {
  ...
  'Codex': { primary: '#facc15', secondary: '#78350f', accent: '#fef08a', bg: '#0c0a09', glow: 'rgba(250, 204, 21, 0.4)' },
};
...
const App: React.FC = () => {
  ...
  const [ttsAudio, setTtsAudio] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    if (!loading && viewUrl && isAutoPanning && !isHoverPaused) {
      const stability = liveSensorData?.raiStability ?? 1;
      const baseSpeed = 0.04;
      // Time dilation: auto-pan slows down when stability is low
      const speed = baseSpeed * (0.3 + 0.7 * stability); 
      const interval = setInterval(() => {
        setScrollPos(prev => (prev + speed) % 100);
      }, 50);
      return () => clearInterval(interval);
    }
  }, [loading, viewUrl, isAutoPanning, isHoverPaused, liveSensorData?.raiStability]);
  ...
  const handleNarrateBriefing = async () => {
    if (!locationInfo || isNarrating) return;
    const text = `Mission Briefing for ${locationInfo.name}. Strategic Context: ${locationInfo.strategicContext}. Threat Level: ${locationInfo.threatLevel}.`;
    handleVoiceBroadcast(text);
  };
  ...
  const handleVoiceBroadcast = async (text: string) => {
    ...
    try {
      ...
      console.error("RAI Narration Failed:", err);
    } finally {
  ...
  if (isApiKeySelected === false || quotaExceeded) {
    return (
      <div className="h-screen w-screen bg-[#020617] flex items-center justify-center p-6 relative overflow-hidden">
        <div className="absolute inset-0 blueprint-grid opacity-20"></div>
        <div className="absolute inset-0 blueprint-subgrid pointer-events-none opacity-10"></div>
        ...
        <div className="max-w-md w-full bg-slate-900/80 border border-cyan-500/30 backdrop-blur-xl p-8 rounded-2xl shadow-[0_0_50px_rgba(34,211,238,0.1)] z-10 flex flex-col items-center text-center">
          <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 border shadow-[0_0_20px_rgba(34,211,238,0.2)] ${quotaExceeded ? 'bg-red-500/10 border-red-500/50' : 'bg-cyan-500/10 border-cyan-500/50'}`}>
            <span className="text-4xl">{quotaExceeded ? '⚠️' : '🔑'}</span>
          </div>
          
          <h1 className={`font-orbitron font-bold text-2xl mb-2 uppercase tracking-tighter ${quotaExceeded ? 'text-red-400' : 'text-cyan-400'}`}>
            {quotaExceeded ? 'RAI Core Quota Exceeded' : 'Uplink Authorization Required'}
          </h1>
          <p className="text-slate-400 text-sm mb-8 font-mono leading-relaxed">
            {quotaExceeded 
              ? "The shared RAI Core uplink has reached its limit (429). To continue analyzing locations without interruption, please select your own paid Google Cloud API key."
          ...
          <div className="w-full space-y-4">
            <button 
              ...
              className={`w-full py-4 font-orbitron font-bold rounded-xl shadow-[0_0_20px_rgba(34,211,238,0.4)] transition-all active:scale-95 uppercase tracking-widest ${quotaExceeded ? 'bg-red-600 hover:bg-red-500 text-white' : 'bg-cyan-500 hover:bg-cyan-400 text-black'}`}
            >
              {quotaExceeded ? 'Connect Paid API Key' : 'Select API Key'}
            </button>
            ...
            {quotaExceeded && (
              <button 
                ...
                className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 font-orbitron font-bold rounded-xl transition-all uppercase tracking-widest text-[10px]"
              >
                Retry Handshake
              </button>
            )}
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col bg-[#020617] text-slate-200 blueprint-grid relative">
      ...
      <div className="scanline"></div>
      {locationInfo && (
        <AudioManager profile={locationInfo.soundProfile} status={lockStatus} scrollPos={scrollPos} isMuted={isMuted} ttsAudio={ttsAudio} />
      )}
      ...
      <main className="flex-1 flex overflow-hidden relative">
        <aside className="w-80 border-r border-cyan-500/20 bg-slate-950/80 backdrop-blur-xl p-4 flex flex-col gap-6 z-10 overflow-y-auto custom-scrollbar blueprint-border">
          ...
          <MissionBriefing location={locationInfo} loading={loading} onNarrate={handleNarrateBriefing} isNarrating={isNarrating} style={selectedStyle} />
          ...
          <AnimatePresence>
            {showStrategicAnalysis && (
              <StrategicAnalysis 
                locationName={locationInfo?.name || INITIAL_LOCATION} 
          ...
          <BotInterface 
            ...
            onBroadcast={handleVoiceBroadcast}
            isBroadcasting={isNarrating}
            onOpenStrategicAnalysis={() => setShowStrategicAnalysis(true)}
          />
            ...
            >
              ...
              <div className="absolute inset-0 flex transition-all duration-500 ease-out" style={{ transform: `translateX(-${scrollPos}%) translateY(${verticalPan}%) scale(${zoom})` }} >
                <div className="relative h-full min-w-[300vw]">
                  {/* Time Dilation Effect based on RAI Stability */}
                  <img 
                    src={viewUrl} 
                    alt="360 Perspective" 
                    className="h-full object-cover w-full brightness-110 contrast-110 pointer-events-none select-none image-pulse-vitals transition-all duration-1000" 
                    style={{
                      filter: `
                        blur(${liveSensorData ? (1 - liveSensorData.raiStability) * 4 : 0}px) 
                        hue-rotate(${liveSensorData ? (1 - liveSensorData.raiStability) * 15 : 0}deg)
                        saturate(${liveSensorData ? 1 + (1 - liveSensorData.raiStability) * 0.5 : 1})
                        brightness(1.1) 
                        contrast(1.1)
                      `,
                      transform: `scale(${liveSensorData ? 1 + (1 - liveSensorData.raiStability) * 0.03 : 1})`,
                    }}
                  />
                  
                  {/* 3D City Model Overlay */}
                  {explorationMode === '3D City Model' && !loading && !imageLoading && (
                    <div className="absolute inset-0 pointer-events-none z-20 mix-blend-screen opacity-30">
                      ...
                      <div className="absolute inset-0 blueprint-subgrid opacity-20"></div>
                      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,rgba(0,0,0,0.4)_100%)]"></div>
                      {/* Floating Data Points Simulation */}
                  ...
                  {explorationMode === 'Satellite View' && !loading && !imageLoading && (
                    <div className="absolute inset-0 pointer-events-none z-20 mix-blend-overlay opacity-40">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="w-[80%] h-[80%] border border-cyan-500/20 rounded-full animate-[pulse_4s_ease-in-out_infinite]"></div>
                        <div className="absolute w-[60%] h-[60%] border border-cyan-500/10 rounded-full"></div>
                      </div>
                      <div className="absolute top-10 left-10 flex flex-col gap-1">
                        <div className="text-[8px] font-mono text-cyan-400/60 uppercase tracking-widest">Orbital_Sync: ACTIVE</div>
                        <div className="text-[8px] font-mono text-cyan-400/60 uppercase tracking-widest">Alt: 35,786km</div>
                      </div>
                      <div className="absolute bottom-10 right-10 flex flex-col gap-1 text-right">
                        <div className="text-[8px] font-mono text-cyan-400/60 uppercase tracking-widest">Lat: {locationInfo?.coordinates.lat.toFixed(4)}</div>
                        <div className="text-[8px] font-mono text-cyan-400/60 uppercase tracking-widest">Lng: {locationInfo?.coordinates.lng.toFixed(4)}</div>
                      </div>
                    </div>
                  )}
